                        ...
                        <div id="filter-list" class="filter-values controls">
                            ...
                            if ( $featured_posts ):
                                ...
                                foreach ( $featured_posts as $post ):
                                    ...
                                    ?>
                                    <?php
                                        $wp_day[] = get_field( 'w_day' );
                                        $unique_dates = array_unique($wp_day);
                                        $unique_dates = array_values(array_filter($unique_dates));
                                ...
                                foreach ( $unique_dates as $unique_date ):
                                    $date_c++;
                                    ?>
                                    <button type="button" id="DayBtn<?php echo $date_c; ?>" class="filter-value control <?php if( 1 == $date_c ): ?>mixitup-control-active<?php endif; ?> text-white date-<?php echo $date_c; ?>" data-filter=".day<?php echo $date_c; ?>">Day <?php echo $date_c; ?>, <?php echo $unique_date; ?></button>
                                <?php endforeach;  wp_reset_postdata(); ?>
                                <?php $date_c = $date_c+1; ?>
                                <?php if( 1 == $on_demand ): ?>
                                <button type="button" id="DayBtn<?php echo $date_c; ?>" class="filter-value control text-white" data-filter=".on_demand">On-Demand</button>
                                <?php endif; ?>
                            </div>
                        </div>
                        

                            <div class="session-grid row scroll" data-target=".session">
                                ...
                                if ( $featured_posts ):
                                    ...
                                    foreach ( $featured_posts as $post ):
                                        ...
                                        <?php get_template_part( 'template-parts/single-event/session-card-filtering' ); ?>
